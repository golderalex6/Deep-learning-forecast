...
class timeseries_rnn(funtional):
    ...
    def train(self,epochs=10,optimizer='adam',loss='mae',batch_size=32):
        ...
        best_lost=tf.keras.callbacks.ModelCheckpoint(os.path.join(Path().cwd(),f'models/rnn_{self.price_type}_{self.symbol}_{self.timeframe}.weights.h5'),save_weights_only=True,monitor='loss',mode='min',save_best_only=True)
